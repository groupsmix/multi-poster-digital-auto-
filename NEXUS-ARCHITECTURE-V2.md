...
## PART 1 — INFRASTRUCTURE (RECOMMENDED)

### Why 100% Cloudflare Stack

You already pay $5/month for Workers Paid plan.
Everything below is included or near-free.
No Supabase. No Vercel. No extra bills. One platform. One dashboard.

```
┌─────────────────────────────────────────────────────┐
│              CLOUDFLARE $5/MONTH PLAN                │
│                                                       │
│  Pages          → Frontend hosting (free)            │
│  Workers        → All backend logic (included)       │
│  D1             → SQL database — 5GB free            │
│  R2             → File storage — 10GB free           │
│  KV             → Fast config/settings storage       │
│  Durable Objects→ Stateful workflow coordination     │
│  Queues         → Async workflow step execution      │
│  Images         → Image CDN + transformation         │
│  Secrets Store  → All API keys, encrypted            │
└─────────────────────────────────────────────────────┘
```
...
| Layer | Service | Job |
|-------|---------|-----|
| Frontend | Cloudflare Pages + Next.js | The dashboard UI |
| API Router | Cloudflare Workers (Hono.js) | Routes all requests |
| Workflow Engine | Durable Objects | Manages stateful multi-step workflows. If tab closes, workflow continues. |
| Task Queue | Cloudflare Queues | Sends each workflow step to a Worker asynchronously |
| Database | D1 (SQLite) | Stores everything: domains, categories, products, prompts, reviews, history |
| File Storage | R2 | Stores: generated PDFs, images, audio files, exports |
| Image CDN | Cloudflare Images | Serves + transforms all product images |
| Config Cache | KV | Fast reads for: active AI models, prompt templates, platform rules |
| Secrets | Cloudflare Secrets Store | All AI API keys, encrypted, never in code |

### Deletion Flow (Dashboard → Cloudflare)

When you delete anything from the dashboard:

```
You click DELETE on dashboard
         │
         ▼
Worker receives DELETE request
         │
    ┌────┴────────────────────────────┐
    │  Delete from D1 (database row)  │
    │  Delete from R2 (files/assets)  │
    │  Delete from KV (cached config) │
    │  Delete from CF Images (if img) │
    └─────────────────────────────────┘
         │
         ▼
...
Everything synced. Nothing orphaned.
