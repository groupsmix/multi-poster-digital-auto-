/**
 * Simple SVG icons for the sidebar and empty states.
 * Each returns an inline SVG string (24×24, stroke-based).
 */
...
export const ICONS = {
  ...
  workflow: svg('<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/><line x1="12" y1="2" x2="12" y2="22"/>'),
...
/** Return a larger version of an icon for empty states */
export function emptyIcon(key: keyof typeof ICONS): string {
  const paths = ICONS[key].replace(/width="\d+"/, 'width="48"').replace(/height="\d+"/, 'height="48"');
  return `<div class="empty-state-icon">${paths}</div>`;
}
